...
contract ERC20AC{
    ...
    function decimals() external pure returns(uint) { assembly {
        mstore(0x00, 0x12)
        return(0x00, 0x20)
    }}

    function allowance(address adr, address ad2) external view returns(uint) { assembly {
        mstore(0x00, adr)
        mstore(0x20, ad2)
    ...
    function balanceOf(address adr) external view returns(uint) { assembly {
        mstore(0x00, sload(adr))
        return(0x00, 0x20)
    }}

    function totalSupply() external view returns(uint) { assembly {
        mstore(0x00, sload(TTS))
        return(0x00, 0x20)
    }}

    function approve(address adr, uint amt) external returns(bool) { assembly {
        mstore(0x00, caller())
        mstore(0x20, adr)
    ...
    function transfer(address adr, uint amt) external returns(bool) { assembly {
    ...
    function transferFrom(address adr, address ad2, uint amt) public returns (bool) { assembly {
        let bal := sload(adr)
        mstore(0x00, adr)
        mstore(0x20, ad2)
